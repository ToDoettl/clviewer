...
fun buildTurnierbaum(saison: Int) = GridPane().apply {
    ...
    val vereine = turnier.get(Phase.ACHTELFINALE).orEmpty().flatMap { it.map { it.daheim } }.distinct()
...
private fun GridPane.buildSpiel(paarung: List<Spiel>?, index: Int, wappen: Map<Verein?, Image>) = row {
    val daheim = paarung?.get(index)?.daheim ?: Unbekannt
    val auswaerts = paarung?.get(index)?.auswaerts ?: Unbekannt
